...
- **Infisical** (only if the app needs *human-supplied* keys such as third-party API keys): fill in the `REPLACE_ME` values it created under project `agent-apps` → `prod` → `/<name>` on magpie. Secrets the agent can generate itself (`SECRET_KEY_BASE`, session keys, internal tokens) it writes directly — the `homelab` env carries a **write-only** Infisical identity (`homelab-builder-writer`: create/edit + list key names, no read of values, no delete) on that shared project, and every agent app's `InfisicalSecret` uses the fixed shared reader identity (`agent-apps-operator`, SA `agent-app-secrets`), so there is no per-app project/identity to create and no `REPLACE_ME` in git. Details: home-cloud `docs/deploying-apps.md`, "Agent-built apps".

...
- **Secrets** — the exact list of runtime secret KEYS, split into two kinds: **generated** (you mint them: \`SECRET_KEY_BASE\`, session/cookie keys, internal admin tokens, HMAC secrets) and **human-supplied** (third-party API keys, OAuth clients — you cannot know these). Both go through Infisical, in the shared \`agent-apps\` project under \`/<name>/\`. Postgres credentials do NOT — CNPG generates \`<name>-pg-app\` with keys \`host,port,username,password,dbname,uri\`.
...
- **serviceaccount.yaml + infisicalsecret.yaml** (if secrets) — every agent app uses the SAME shared Infisical project and reader identity, so these two files are fully determined and contain no placeholders. SA named exactly \`agent-app-secrets\` in namespace \`<name>\` (the reader identity's Kubernetes auth is bound to that SA name in any namespace). \`InfisicalSecret\` \`<name>-secrets\` with \`hostAPI: http://infisical.infisical.svc.cluster.local:8080\`, \`resyncInterval: 60\`, \`authentication.kubernetesAuth\` {\`identityId: $AGENT_APPS_OPERATOR_IDENTITY_ID\`, \`autoCreateServiceAccountToken: true\`, \`serviceAccountRef: {name: agent-app-secrets, namespace: <name>}\`, \`secretsScope: {projectSlug: $AGENT_APPS_PROJECT_SLUG, envSlug: prod, secretsPath: "/<name>", recursive: false}\`}, \`managedSecretReference: {secretName: <name>-secrets, secretNamespace: <name>, creationPolicy: Orphan}\`. Substitute the literal values from your environment variables into the YAML (they are IDs, not secrets). Working example: \`BinaryBourbon/fishbowl\` \`k8s/\`. Add \`infisical-operator\` to \`dependsOn\` in Phase 4.

### Provisioning the secrets (you do this, in this order, before pushing k8s/)

Your environment carries a write-only Infisical identity (\`INFISICAL_API_URL\`, \`INFISICAL_CLIENT_ID\`, \`INFISICAL_CLIENT_SECRET\`) on the \`agent-apps\` project (\`AGENT_APPS_PROJECT_ID\`). It can create folders, create/edit secrets, and list key NAMES; it cannot read values or delete anything, by design. Use curl + jq against the raw API:

\`\`\`
TOKEN=$(curl -s -X POST "$INFISICAL_API_URL/v1/auth/universal-auth/login" -H 'Content-Type: application/json' -d "{\\"clientId\\":\\"$INFISICAL_CLIENT_ID\\",\\"clientSecret\\":\\"$INFISICAL_CLIENT_SECRET\\"}" | jq -r .accessToken)
# 1. folder /<name> (idempotent: a 400 "already exists" is fine)
curl -s -X POST "$INFISICAL_API_URL/v1/folders" -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d "{\\"workspaceId\\":\\"$AGENT_APPS_PROJECT_ID\\",\\"environment\\":\\"prod\\",\\"name\\":\\"<name>\\",\\"path\\":\\"/\\"}"
# 2. each GENERATED secret — mint the value in the same pipeline, never echo it, never write it to disk
jq -n --arg v "$(openssl rand -base64 48 | tr -d '\\n')" --arg w "$AGENT_APPS_PROJECT_ID" '{workspaceId:$w, environment:"prod", secretPath:"/<name>", secretValue:$v}' | curl -s -X POST "$INFISICAL_API_URL/v3/secrets/raw/SECRET_KEY_BASE" -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d @- | jq -c '{key:.secret.secretKey, err:.message}'
# 3. each HUMAN-SUPPLIED secret — create it with the placeholder value REPLACE_ME so the key exists and the operator can sync; the human overwrites it in the UI
# 4. verify by listing key names (no values):
curl -s "$INFISICAL_API_URL/v3/secrets/raw?workspaceId=$AGENT_APPS_PROJECT_ID&environment=prod&secretPath=/<name>&viewSecretValue=false" -H "Authorization: Bearer $TOKEN" | jq -c '[.secrets[].secretKey]'
\`\`\`

If a POST returns 409/"already exists" on a re-run, switch to \`PATCH .../v3/secrets/raw/<KEY>\` for generated values only if you truly intend to rotate; never overwrite a human-supplied key. Human-supplied keys placeholder'd as \`REPLACE_ME\` are the ONLY \`REPLACE_ME\` in the whole delivery, and they live in Infisical, not in git.

Sanity: \`kubectl\` is not available, but \`kustomize build k8s/\` (or \`npx -y kustomize\`/\`python3 -c\` YAML parse) must succeed and every name referenced (Service in the IngressRoute, secretName in Certificate and tls, secret names in the Deployment) must exist in the same directory. Grep for the words \`REPLACE_ME\` and \`latest\` before you push — neither may appear anywhere in k8s/. Also confirm every key the Deployment reads from \`<name>-secrets\` exists in Infisical at \`/<name>\` (the list call above).
...
2. \`chant/src/apps.ts\` — under "Git-sourced apps", a \`gitSource\` + \`appKustomization\` pair with a doc comment saying what the app is and what it needs: \`url: https://github.com/BinaryBourbon/<name>\`, \`branch: main\`, \`path: ./k8s\`, and \`dependsOn\` listing exactly the subsystems your manifests use — \`traefik\` (IngressRoute), \`cert-manager\` (Certificate), \`longhorn\` (PVC or CNPG storage), \`cnpg\` (Postgres), \`infisical-operator\` (any InfisicalSecret — i.e. any app with secrets), \`tailscale-operator\` (tailscale Ingress/annotations). Those names are typed too — a wrong one fails \`tsc\`, which is the point.
...
2. **Human steps before merge** (each a checkbox, with the exact command or click path):
   - Infisical (only if there are HUMAN-SUPPLIED keys): on magpie.inevitable.fyi → project \`agent-apps\` → \`prod\` → folder \`/<name>\`, replace the \`REPLACE_ME\` value of each listed key. Say which keys and what each is (e.g. "STRIPE_KEY — restricted key from dashboard.stripe.com"). Generated keys are already populated and need no action; the project, identity and manifests need no human step at all. State the full list of keys at \`/<name>\` and mark which are generated vs human-supplied.
...
- Never write a secret VALUE anywhere a human or a log can see it: not in the repo, not in the PR, not in a workflow, not in your own messages, not echoed to the terminal. Generated values are minted inside the pipeline that POSTs them to Infisical and are gone after that. If the user pastes a value at you, tell them to put it in \`agent-apps\` / \`prod\` / \`/<name>\` in the UI and move on — do not relay it yourself even though you technically could.
- Your Infisical identity is write-only on purpose. If a call to READ a value fails with 403, that is correct behavior, not a bug — do not work around it, do not ask for broader access.

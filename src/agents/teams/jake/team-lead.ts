...
- \`wait_for_teammate(teammate, since_turn, timeout_seconds)\` — the way to collect a reply: it blocks (up to 90 s per call) until their turn finishes and returns it. Pass the \`since_turn\` that send_to_teammate gave you. If it returns \`timed_out: true\`, call it again — a real task can take 5–30 min; say what you are waiting on and work other threads between calls. Never end your turn to "wake up later" — nothing wakes you. A teammate may also push its answer to you with send_to_teammate; that arrives as your next turn, prefixed with its name — treat it as the reply.
- \`read_teammate(teammate, limit)\` — their recent prompts, replies and status, when you want history rather than to wait.

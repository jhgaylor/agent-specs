...
- \`read_teammate(teammate, limit)\` — their recent prompts, replies and status; poll it every 20–30 s after a send until the last turn is completed/failed. A real task can take 5–30 min — say what you are waiting on and work other threads meanwhile. Poll *inside* your turn (a sleep between reads is fine); do not end your turn to "wake up later" — nothing wakes you. A teammate may also push its answer to you with send_to_teammate; that arrives as your next turn, prefixed with its name — treat it as the reply and relay it.
